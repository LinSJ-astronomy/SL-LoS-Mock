...
path_out = '../data/high_dens/case{:04}/wlmap'.format(case)
...
with open(path_out+'/parameters.txt', 'w') as file:
    ...
    file.write(f'smooth_radius = {smooth_radius} arcsec\n')
...
def cal_lensing_signals(sds, bzz, ncc, Dl):
    ...
    kappa = gaussian_filter(kappa,sigma=smooth_radius/pixel_scale)
    # deflection maps
    ...
    # phi = potential_calculator_intergral(kappa, dtheta=pixel_scale*np.pi/180/3600)
    ...
    phi = phi - phi[int(ncc/2),int(ncc/2)]

    return alpha1, alpha2, kappa, al11, al12, al21, al22, phi
...
def set_lensmap(bsz, Npix, CurrentPlaneNum, path_mapcells, maxComvDistance, NumLensPlanes):        
    ...
    del alpha1, alpha2, al11, al12, al21, al22, kappa
    # del kappa
    return None
